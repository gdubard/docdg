...
#[test]
fn rend_basique2_sans_panique() {
    let src = include_str!("../../exemples/basique2.txt");
    ...
    assert_eq!(r.page.marges, [5.0, 5.0, 5.0, 5.0]);
}
...
fn decalage_des_zones_de_texte() {
    ...
    assert!(r.html.contains("width:0.8cm\"></span>Décalé d'une tabulation."), "{}", r.html);
    assert!(r.html.contains("width:1.6cm\"></span>Décalé de deux."), "{}", r.html);
    assert!(r.html.contains("width:0.8cm\"></span>Décalé de quatre espaces."), "{}", r.html);
...
fn decalage_ligne_par_ligne_et_en_cours_de_ligne() {
    ...
    assert_eq!(r.html.matches("<p>").count(), 1, "{}", r.html);
    assert!(r.html.contains("<br><span style=\"display:inline-block;width:0.8cm\"></span>Deuxième"), "{}", r.html);
    let t = e.render("Un\tmot, puis\t\t\ttrois.", false);
    assert!(t.html.contains("width:0.8cm\"></span>mot"), "{}", t.html);
    assert!(t.html.contains("width:2.4cm\"></span>trois."), "{}", t.html);
}
...
#[test]
fn une_consigne_se_poursuit_apres_une_virgule() {
    // Sans cela, une énumération d'attributs un peu longue était condamnée à
    // tout enfermer entre les chevrons — deux écritures pour une seule
    // notion. C'est la ponctuation française qui décide : la virgule appelle
    // la suite.
    let mut e = Engine::new();
    let r = e.render(
        "<Affiche>un cadre avec une bordure rouge,\n  un fond blanc{\nTexte.\n}",
        false,
    );
    assert!(!r.html.contains("calcul-absent"), "{}", r.html);
    assert!(r.html.contains("Texte."), "{}", r.html);
}

#[test]
fn une_consigne_se_poursuit_sur_un_bloc_en_retrait() {
    // Les deux-points annoncent des valeurs, et un bloc indenté appartient
    // visiblement à la consigne qui le porte.
    let mut e = Engine::new();
    let r = e.render(
...
#[test]
fn un_alinea_apres_une_consigne_reste_de_la_prose() {
    // La poursuite ne s'amorce que sur une virgule ou des deux-points : une
    // consigne ordinaire suivie d'un alinéa n'absorbe pas le paragraphe.
    let mut e = Engine::new();
    let r = e.render("<Soit>une fonction f(x) = x^2\n\tUn alinéa de prose.", false);
    assert!(r.html.contains("Un alinéa de prose."), "{}", r.html);
    assert!(!r.html.contains("calcul-absent"), "{}", r.html);
}

#[test]
fn le_nom_declare_s_arrete_a_l_accolade() {
    // `<Soit>la matrice M{` nommait la matrice « M{ » : tout calcul qui la
    // rappelait ensuite ne la trouvait plus.
    let mut e = Engine::new();
    let r = e.render(
