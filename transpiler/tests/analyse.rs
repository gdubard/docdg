...
fn analyse_college_complet() {
    let src = include_str!("../../exemples/analyse2.txt");
...
fn analyse_lycee_complet() {
    let src = include_str!("../../exemples/analyse3.txt");
    let mut e = Engine::new();
    let r = e.render(src, true);
    assert!(!r.html.contains("calcul-absent"));
...
fn analyse_superieur_complet() {
    let src = include_str!("../../exemples/analyse4.txt");
    let mut e = Engine::new();
    let r = e.render(src, true);
    assert!(!r.html.contains("calcul-absent"));
...
fn les_deux_modes_de_rendu_donnent_le_meme_document() {
    for source in [
        include_str!("../../exemples/factorisation.txt"),
        include_str!("../../exemples/analyse3.txt"),
        include_str!("../../exemples/geometrie3.txt"),
        include_str!("../../exemples/statistiques-probabilites3.txt"),
    ] {
...
fn les_courbes_etudiees_portent_tangentes_et_asymptotes() {
    for parallele in [false, true] {
        let h = Engine::new()
            .render(include_str!("../../exemples/factorisation.txt"), parallele)
            .html;
