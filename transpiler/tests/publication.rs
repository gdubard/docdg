...
fn la_police_du_document_se_declare_par_son_nom() {
    let j = page_json("page {\n\tscript: Latin Modern Roman;\n\ttaille: 12;\n}\nBonjour.");
    assert!(j.contains("\"script\":\"Latin Modern Roman\""));
...
fn les_nouvelles_cles_ont_des_defauts_stables() {
    ...
    // Le carreau et l'interligne de la Seyès : c'est la réglure de l'école
    // élémentaire, et la seule valeur qui n'ait jamais besoin d'être écrite.
    assert!(j.contains("\"tabulation\":8"));
    assert!(j.contains("\"hauteur\":8"));
...
fn la_tabulation_pilote_l_indentation() {
    ...
    assert!(defaut.contains("width:0.8cm"), "{}", defaut);
}
...
fn la_hauteur_pilote_la_ligne_vide() {
    ...
    let defaut = rend("Un.\n\nDeux.");
    assert!(defaut.contains("height:0.8cm"), "{}", defaut);
}
...
fn les_reglages_ne_fuient_pas_entre_documents() {
    ...
    assert!(apres.contains("width:0.8cm"), "{}", apres);
...
fn le_bloc_document_fusionne_metadonnees_et_reglages() {
    ...
    assert!(j.contains("\"script\":\"Georgia\""), "{}", j);
