...
fn blocs_secables_marques() {
    ...
    let src = include_str!("publication3.txt");
...
fn alinea_rendu_dans_les_blocs() {
    ...
    assert!(cadre.html.contains("width:1cm"), "{}", cadre.html);
    let libre = e.render("\tAlinéa au fil du texte.", false);
    assert!(libre.html.contains("width:1cm"), "{}", libre.html);
    let plat = e.render("<Affiche>un cadre avec un fond jaune{\nSans alinéa.\n}", false);
    assert!(!plat.html.contains("width:1cm"), "{}", plat.html);
}
...
fn algo2_boucles_filtrees_diviseurs_ternaire_et_ou() {
    ...
    assert!(r.html.contains("17") && r.html.contains("impair"));
    assert!(r.html.contains(">15<") && r.html.contains(">30<"));
