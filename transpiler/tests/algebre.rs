...
fn algebre_college_complet() {
    let src = include_str!("../../exemples/algebre2.txt");
...
fn algebre_lycee_complet() {
    let src = include_str!("../../exemples/algebre3.txt");
    let mut e = Engine::new();
    let r = e.render(src, true);
    assert!(!r.html.contains("calcul-absent"));
...
fn algebre_superieur_complet() {
    let src = include_str!("../../exemples/algebre4.txt");
    let mut e = Engine::new();
    let r = e.render(src, true);
    assert!(!r.html.contains("calcul-absent"));
...
fn commande_tabulee_garde_son_decalage() {
    ...
    // Une tabulation vaut un carreau : huit millimètres, la réglure de
    // l'école élémentaire, depuis que l'écriture sur des lignes en a fait
    // l'unité de retrait du document.
    assert!(r.html.contains("margin-left:0.8cm"), "{}", r.html);
