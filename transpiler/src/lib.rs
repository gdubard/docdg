...
impl Engine {
    ...
    pub fn render(&mut self, src: &str, parallel: bool) -> RenderResult {
        ...
        layout::rendu::set_police_seyes(&page.seyes);
        ...
        let sceau = {
            let mut h = seahash::SeaHasher::new();
            page.tabulation.to_bits().hash(&mut h);
            page.hauteur.to_bits().hash(&mut h);
            page.precision.hash(&mut h);
            page.seyes.hash(&mut h);
            h.finish()
        };
        let mut base = empreinte_env(&env) ^ sceau;
        ...
        let contenance: usize = cles
            .iter()
            .filter_map(|c| self.cache.get(c))
            .map(|e| e.html.len())
            .sum();
        let mut html = String::with_capacity(contenance + 256);
        let ligne_vide = format!(
            "<div class=\"ligne-vide\" style=\"height:{}cm\"></div>",
            layout::rendu::reglages_page().hauteur_cm
        );
        let mut vides = 0usize;
        ...
        let mut html = finalise(&html, &toc_html);

        if html.contains(layout::ecriture::CLASSE) {
...
mod invariant {
    ...
    const CORPUS: &[(&str, &str)] = &[
        ("basique1", include_str!("../../exemples/basique1.txt")),
        ("basique2", include_str!("../../exemples/basique2.txt")),
        ("basique3", include_str!("../../exemples/basique3.txt")),
        ("algebre2", include_str!("../../exemples/algebre2.txt")),
        ("algebre3", include_str!("../../exemples/algebre3.txt")),
        ("algebre4", include_str!("../../exemples/algebre4.txt")),
        ("analyse2", include_str!("../../exemples/analyse2.txt")),
        ("analyse3", include_str!("../../exemples/analyse3.txt")),
        ("analyse4", include_str!("../../exemples/analyse4.txt")),
        ("geometrie2", include_str!("../../exemples/geometrie2.txt")),
